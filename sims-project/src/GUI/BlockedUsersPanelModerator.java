...
public class BlockedUsersPanelModerator extends JPanel {


    ToolBarPanelModerator parentPanel ;

    public BlockedUsersPanelModerator(ToolBarPanelModerator parenPanel) {
    ...
    public void initBlockedUsersPanel() {
        ...
        // Naslov "BlockedUsers"
        JLabel usernameLabel = new JLabel("BlockedUsers");
        ...
        for (int i = 0; i < usersCount; i++) {
            JPanel userPanel = createBlockedUserPanel("Cover " + (i + 1), "User " + (i + 1));
    ...
    private JPanel createBlockedUserPanel(String coverText, String username) {
        ...
        panel.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                int result = JOptionPane.showOptionDialog(null, // roditelj komponenta, null za centriranje
                        "Do you want to unblock user?", // poruka koja se prikazuje
                        "Activate user", // naslov messagebox-a
    ...
    private JTextField createSearchBar() {
        JTextField searchBar = new JTextField("      Search blocked users"){
            @Override protected void paintComponent(Graphics g) {
                if (!isOpaque() && getBorder() instanceof RoundBorder) {
                    Graphics2D g2 = (Graphics2D) g.create();
                    g2.setPaint(getBackground());
