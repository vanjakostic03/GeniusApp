...
public class BandsPanel extends JPanel {
    ...
    ArrayList<Bend> artists = new ArrayList<>() ;

    public void setAlbums(ArrayList<Bend> artists) {
        this.artists = artists;
    }

    public BandsPanel(ToolBarPanelUser parenPanel, ArrayList<Bend> artists) {
    ...
    public void initArtistPanel() {
        ...
        // Naslov "Artists"
        JLabel titleLabel = new JLabel("Bands");
    ...
    private JTextField createSearchBar() {
        JTextField searchBar = new JTextField("      Search bands"){
            @Override protected void paintComponent(Graphics g) {
                if (!isOpaque() && getBorder() instanceof RoundBorder) {
                    Graphics2D g2 = (Graphics2D) g.create();
                    g2.setPaint(getBackground());
